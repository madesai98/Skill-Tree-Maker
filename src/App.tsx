...
function normalizeStats(raw: unknown): StatDefinition[] {
  ...
  return raw.flatMap((item, index) => {
    ...
    return [{
      ...
      key: typeof value.key === 'string' ? value.key : `stat.${index + 1}`,
...
function SkillTreeEditor() {
  ...
  const addStat = () => {
    ...
    setStats((current) => [
      ...current,
      { id, key: `stat.${current.length + 1}`, name: 'New Stat', type: 'number' },
    ]);
  };
        ...
        <section className="stat-pool-view">
          <div className="stat-pool-head">
            <div>
              ...
              <h2>Stat pool</h2>
              <p>Define the typed stat lines that skill effects are allowed to modify.</p>
            </div>
            <button className="primary-button" onClick={addStat}><Icon name="plus" /> Add stat</button>
          </div>
          ...
          <div className="stat-table-card">
            ...
            {stats.length === 0 ? (
              <div className="stat-empty"><h3>No stats configured</h3><p>Add a number or toggle to make it available in skill effects.</p></div>
            ) : stats.map((stat) => {
              ...
              return (
                <div className="stat-row" key={stat.id}>
                  ...
                  <label><span className="mobile-label">Game key</span><input className="mono-input" value={stat.key} onChange={(e) => updateStat(stat.id, { key: e.target.value })} /></label>
